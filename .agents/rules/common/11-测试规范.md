---
alwaysApply: false
description: 项目的测试规范，包括测试框架选型、文件约定、测试范围、编写规范与质量门禁。当编写测试或评审测试代码时读取此规则。
---
...
| 组件测试 | `<name>.test.tsx`（React）/ `<name>.test.ts`（Vue） |
| 公共 Mock / 工具 | `tests/` 或 `src/__tests__/helpers/` |
| 测试配置 | 项目根目录 `vitest.config.ts` |
...
- 所有代码必须通过 **TypeScript 类型检查**（`tsc --noEmit`）
- 所有代码必须通过 **ESLint 检查**（零 error）
