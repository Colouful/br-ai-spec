---
alwaysApply: false
description: 项目的路由规范，包括路由配置结构、懒加载、导航守卫、路由 meta 定义。当新增页面或配置路由时读取此规则。
---

# 路由规范（NON-NEGOTIABLE）

## 路由结构

- 路由配置统一在 `src/router/` 目录下管理
- 页面视图统一在 `src/views/` 目录下，使用 `kebab-case` 命名
- 每个页面目录包含 `index.vue` 作为入口组件

```
src/router/
├── index.ts           # 创建路由实例、注册全局守卫
├── routes.ts          # 路由表（或按模块拆分为 modules/）
└── modules/           # 可选：按业务模块拆分路由配置
    ├── dashboard.ts
    └── settings.ts
```

## 懒加载（NON-NEGOTIABLE）

所有页面级路由必须使用动态导入实现懒加载：

```typescript
{
  path: '/dashboard',
  name: 'Dashboard',
  component: () => import('@/views/dashboard/index.vue'),
  meta: { title: '仪表盘', requiresAuth: true },
}
```

- 禁止在路由配置中静态导入页面组件
- 布局组件（`src/layouts/`）允许静态导入

## 路由 Meta 规范

路由 `meta` 字段用于声明页面元信息，统一类型声明：

```typescript
declare module 'vue-router' {
  interface RouteMeta {
    title?: string
    requiresAuth?: boolean
    roles?: string[]
    keepAlive?: boolean
    hidden?: boolean
  }
}
```

## 导航守卫

- 全局前置守卫在 `src/router/index.ts` 中注册，负责鉴权与页面标题设置
- 禁止在组件内使用 `beforeRouteEnter` 处理鉴权逻辑，鉴权统一收敛到全局守卫
- 组件级守卫（`onBeforeRouteLeave` 等）仅用于页面离开确认等局部场景

## 约束

- **禁止多处维护路由**：路由配置集中在 `src/router/` 管理，禁止散落在其他目录
- **避免重复注册**：同一页面组件只允许被一条路由引用
- **路由命名唯一**：每条路由的 `name` 必须全局唯一
