...
当需要为一个**需求**创建提案（proposal、tasks、spec）时使用本技能。需求可能是：
...
- **已提供接口**：按 `.agents/rules/03-项目结构.md` 等规范，在 tasks 中安排 `interfaces/`、`http/` 等。
...
- **新页面**：路由目录、Page、Loader、`index.module.scss`、与布局一致的结构；若有分析清单则写「依据 xxx-UI分析清单 实现」。
...
提案文档创建完成后，进入执行阶段时，须遵循 `.agents/rules/12-Superpowers执行规范.md`，按 `.agents/skills/execute-task/SKILL.md` 的四步循环逐条执行 tasks.md。
...
- `.agents/rules/12-Superpowers执行规范.md` - 阶段 2 执行原则
- `.agents/skills/execute-task/SKILL.md` - 阶段 2 执行操作（Superpowers 四步循环）
