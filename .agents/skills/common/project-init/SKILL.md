...
4. 规则生成范围由两类信号共同决定：
   - `.agents/rules/profiles/<profile>/` 中规则文件是否缺失
...
1. 读取 `.ai-spec/manifest.json` 中的 `profile` 字段
2. 从 `.agents/registry/profiles.json` 中找到对应 profile 条目
3. 使用该条目的 `project_init_rule_ids` 字段确定"应按项目事实生成的能力规则集合"
...
- 始终生成或刷新：
  - `.agents/rules/profiles/<profile>/01-项目概述.md`
  - `.agents/rules/profiles/<profile>/03-项目结构.md`
  - `context/PROJECT.md`
- 若项目已安装 OpenSpec（存在 `openspec/`），同步 `openspec/project.md` 中的项目概述
- 对于安装时选择了"根据项目自定义"的规则，按项目事实生成或刷新 profile 对应的能力规则

## 完成标准（强约束）

以下条件必须同时满足，才算本次 `project-init` 完成：

1. `01-项目概述.md` 已生成或刷新（写入 `.agents/rules/profiles/<profile>/`）
2. `03-项目结构.md` 已生成或刷新（写入 `.agents/rules/profiles/<profile>/`）
3. `context/PROJECT.md` 已生成或刷新
4. 若存在 `openspec/`，则 `openspec/project.md` 已同步
5. 若 profile 对应的能力规则目录中缺失任意应有规则，则这些缺失项必须在本次执行中一并补生成
...
### 第零步：确定 profile 与规则生成范围

- 读取 `.ai-spec/manifest.json` 中的 `profile` 字段
- 从 `.agents/registry/profiles.json` 中查询该 profile 的 `project_init_rule_ids`
...
- 生成或刷新（写入 `.agents/rules/profiles/<profile>/`）：
  - `01-项目概述.md`
  - `03-项目结构.md`
  - `context/PROJECT.md`
