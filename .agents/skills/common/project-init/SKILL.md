---
name: project-init
description: 自动分析当前项目的技术栈与目录结构，生成 01-项目概述.md、03-项目结构.md 与 context/PROJECT.md。当需要初始化项目规范、生成项目概述或填写项目信息时使用本技能。
version: 1.1.0
---
...
### 第四步：生成 context/PROJECT.md

在 `context/` 下生成（或覆盖）`PROJECT.md`，用于提供项目级稳定上下文。

内容定位：

- 只写**稳定项目背景**
- 不写某次任务的 `assumptions（默认假设）`
- 不写短期会失效的扫描结论
- 不写某个页面任务专属的默认落点、默认登录方式之类的运行态信息

严格使用以下模板：

```markdown
---
description: 当前项目的稳定上下文入口，供 task-orchestrator 与各专家优先读取。
---
...
- 项目概述：`../.agents/rules/01-项目概述.md`
- 项目结构：`../.agents/rules/03-项目结构.md`
- API 规范：`../.agents/rules/05-API规范.md`
- 路由规范：`../.agents/rules/06-路由规范.md`
- 样式规范：`../.agents/rules/09-样式规范.md`

## 业务目标

- {可从 README / 项目说明确认的目标；若无法确认，写“待补充”}
...
- {当前明确不覆盖的内容；若无法确认，写“待补充”}
```

**填写规则**：

- `项目定位` 必须与 `01-项目概述.md` 中的 `## 项目定位` 保持一致
- `技术栈摘要` 只写可以从 `package.json` 与代码中确认的事实
- `关键规范入口` 使用相对路径，统一指向 `.agents/rules/`
- `业务目标 / 核心术语 / 非目标` 优先从 `README.md`、现有文档、仓库命名中提取
- 如果无法确认，就明确写 `待补充`，不要编造
- 禁止把“当前仍是 Vite 初始结构”“默认登录方式”“页面默认落点”等任务态信息写入 `PROJECT.md`

### 第五步：同步 openspec/project.md
...
1. 展示检测到的技术栈（表格）
2. 展示检测到的目录结构（树形图）
3. 展示将写入 `context/PROJECT.md` 的项目定位与关键规范入口
4. 展示将写入 `openspec/project.md` 的项目概述描述（如存在）
...
- 如果 `01-项目概述.md`、`03-项目结构.md` 或 `context/PROJECT.md` 已存在且内容非模板（已被用户编辑过），应先展示差异，由用户决定是覆盖还是合并。
- 本技能生成 `01-项目概述.md`、`03-项目结构.md`、`context/PROJECT.md`，并同步 `openspec/project.md` 的项目概述；其他规范文件（02-12）保持原样不动。
