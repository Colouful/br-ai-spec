...
**适用范围（当前版本）**：面向 **前端** 工程，内置 **Vue 3** 与 **React** 两套 Profile（TypeScript、Vite、路由、状态管理与样式约定等）。不包含后端或服务端框架的专用规范模板。
...
**L1 / L2 / L3 是渐进安装层级**：可先只落地规范与 IDE（L1/L2），再在需要 **需求治理与归档闭环** 时升级到 **L3**，与 OpenSpec 一并安装；详见下文「安装层级」与 [docs/openspec-guide.md](docs/openspec-guide.md)。
...
> 首次使用前需配置私有源（仅一次）：在 `~/.npmrc` 中添加 `@bairong:registry=http://nodejs.100credit.cn/`
...
| **L2** | `.agents` + IDE 适配层 + MCP 模板 | 团队编码规范 + 外部上下文（默认安装层级） |
| **L3** | L2 + OpenSpec（`openspec/`、OPSX 命令与产物） | **团队完整方案**：需求提案 → 实施 → 归档闭环，与 `.agents` 通过 `config.yaml` 一体联动 |
