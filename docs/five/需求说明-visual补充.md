# br-ai-spec-visual 需求补充说明

> 文档定位：作为 `需求说明文档.md` 的补充，完整说明 br-ai-spec-visual 可视化监控平台的定位、架构、切面式集成方案和快速部署指南。

## 1. 概述与定位

### 1.1 br-ai-spec-visual 是什么
...
**技术仓库位置**：`/Users/lizhenwei/workspace/vueworkspace/bairong/br-ai-spec-visual`
...
`br-ai-spec-visual` 通过提供 Web 控制台，让运行态"从文件中走出来"，变得可见、可查、可追溯。
...
services:
  visual:
    ...
    container_name: br-ai-spec-visual
    ports:
      - "${PORT:-3000}:3000"
    environment:
      - NODE_ENV=production
  ...
  db:
    image: mariadb:11
    container_name: br-ai-spec-visual-db
    environment:
...
`br-ai-spec-visual-collector` 是一个命令行工具，用于批量扫描项目 `.ai-spec/` 目录并上报到 visual 服务。
...
| 术语 | 全称 | 说明 |
| --- | --- | --- |
| visual | br-ai-spec-visual | 可视化监控平台 |
...
- [快速部署指南（visual 仓库）](../../br-ai-spec-visual/docs/快速部署指南.md)
...
本文档作为 `需求说明文档.md` 的补充，完整说明了 br-ai-spec-visual 的定位、架构和集成方案。
