...
| 路线 | 更偏解决什么问题 | 不足之处 | 当前可借鉴部分 | 当前方案的取向 |
| --- | --- | --- | --- | --- |
| `LangGraph` | 长任务中的状态推进、回环与恢复 | 更偏流程编排框架，本身不直接回答团队规范落盘、项目接入和长期资产沉淀问题 | 状态机、门禁、checkpoint / restore | 将运行状态显式落盘，支持可恢复的协议推进 |
| `Aider` | 代码库感知、最小改动与编辑效率 | 更适合个人或小范围高频改码场景，对团队级交接、审批门禁和归档治理支撑较弱 | 仓库地图、最小改动、验证回灌 | 将实现结果约束在项目规则和交付产物之内 |
| `spec-driven` 路线 | 先沉淀需求和设计，再进入实现 | 更强调产物先行，但对运行态治理、专家协同和项目级安装接入覆盖不够完整 | `proposal / design / tasks / specs` 先行 | 用 OpenSpec 承载交付产物，避免流程只依赖对话 |
| `br-ai-spec` 当前方案 | 团队协作中的规范、状态、资产和归档治理 | 入口体验、远程协同、自动化控制面和维护者排障心智仍需继续打磨 | 组合上述机制，强调团队复用 | 面向项目接入与长期维护，而不是单次编码效率 |
...
当前激活流程为 `prd-to-delivery`。其运行目标是把一条从需求到归档的前端交付链稳定跑通。
...
| 命令 | 作用 |
| --- | --- |
| `protocol-step` | 启动第一轮，或在需要时重新获取当前轮执行说明 |
...
`task-orchestrator -> requirement-analyst -> frontend-implementer -> code-guardian -> before-archive -> archive-change`
...
前者用于阻止需求未收敛时直接进入实现，后者用于在归档前显式确认交付结论。
...
| 闭环 | 从 `/spec-start` 到归档的完成率、门禁阻断点、归档成功率 | 判断流程是否跑完整 |
...
### 7.3 中长期：补齐 OpenClaw 对接，走向 AI 规范驱动自动开发
