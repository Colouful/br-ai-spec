...
`br-ai-spec` 的 P0 依赖 `skill-q-platform` 至少提供稳定 Manifest Export(清单导出)能力；`init --recommend` 依赖 Hub 推荐接口。`br-ai-spec-visual` 只能在 `br-ai-spec` 稳定输出 WorkspaceTopology(工作区拓扑)、RunEvent(运行事件)、History(历史记录)、Incident(事故记录)后完整展示治理面板。
