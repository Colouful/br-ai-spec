...
br-ai-spec 规范库安装工具 v2.0.0，适用于 macOS / Linux / Git Bash / WSL。
...
```
.cursor/skills/
├── create-component/ -> ../../.agents/skills/create-component  # br-ai-spec（链接）
├── create-api/ -> ../../.agents/skills/create-api              # br-ai-spec（链接）
...
这让 OpenSpec 流程自动引用 br-ai-spec 的规范和技能，两者通过 `config.yaml` 一个文件桥接。
...
| 从其他位置运行 | 克隆/更新规范库到 `~/.br-ai-spec/` 缓存 |
