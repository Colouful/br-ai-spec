...
npx @bairong/ai-spec init --profile vue --level L3
```
...
```bash
npx @bairong/ai-spec init --level L3
```
...
运行 `npx @bairong/ai-spec update --level L3`，脚本会检测 `config.yaml` 中是否已有 `context:` 字段，没有才追加，不会覆盖已有配置。
