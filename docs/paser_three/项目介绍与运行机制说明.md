...
> 文档定位：`br-ai-spec` 对外介绍、内部对齐、works 插件“最佳实践文章”基础稿

## 1. 执行摘要

`br-ai-spec` 不是一个“让 AI 自由写代码”的工具集合，而是一套基于 `OpenSpec` 的规范驱动开发底座。它的目标，是把 AI 交付从“临场发挥”升级为“可约束、可追溯、可复用”的团队协作机制。
...
`br-ai-spec` 的价值就在于把这 4 个问题，收敛到一套标准化的工程机制里，并尽量把关键事实落成项目内文件，而不是只停留在对话里。
...
`br-ai-spec` 的正确做法不是替代 OpenSpec，而是把团队执行约束叠加到 OpenSpec 的变更节奏之上。
...
`br-ai-spec` 当前阶段最重要的成果，不是做了多少“AI 功能”，而是把 AI 研发协作沉淀成了一套可管理、可审计、可复制的交付机制。
