...
```mermaid
flowchart LR
    U["用户输入任务"] --> O["task-orchestrator（任务主理人）"]
    O --> P["run-plan（运行计划）"]
    P --> S["run-state（运行状态）"]
...
- `task-orchestrator（任务主理人）` 负责收敛、裁剪和再次分发
...
```mermaid
flowchart TD
    A["原始任务 + 项目上下文 + 规则 + OpenSpec 全量状态"] --> B["task-orchestrator（任务主理人）"]
...
1. `run-state（运行状态）` 落盘
2. `task-orchestrator（任务主理人）` 首轮 `run-plan（运行计划）`
...
> 用 `OpenSpec（规范产物） + run-state（运行状态）` 代替长对话透传，用 `task-orchestrator（任务主理人）` 做最小知情注入和任务锚点重注，让专家协同从“聊天链路”逐步变成“状态驱动链路”。
