...
- `task-orchestrator（任务主理人）` 已经能产出结构化 `run-plan（运行计划）`
...
```bash
ai-spec task-orchestrator-adapter apply \
...
```bash
cat ./.ai-spec/tmp/runtime-action.json | ai-spec task-orchestrator-adapter apply --stdin
```
...
```mermaid
flowchart LR
    A["IDE（开发工具） / OpenClaw（远程入口）"] --> B["task-orchestrator（任务主理人）"]
    B --> C["结构化 payload（载荷）"]
