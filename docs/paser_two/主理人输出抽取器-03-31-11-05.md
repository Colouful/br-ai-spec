...
`task-orchestrator（任务主理人）` 在真实 `IDE（开发工具）` 里，通常不会只回纯 `JSON（结构化数据）`。
...
```mermaid
flowchart LR
    A["task-orchestrator（任务主理人）回复"] --> B["task-orchestrator-extractor（输出抽取器）"]
...
> 主理人把系统动作明确放进 `json` 代码块，再交给抽取器处理。
