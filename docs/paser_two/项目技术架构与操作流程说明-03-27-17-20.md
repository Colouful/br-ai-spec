...
4. `Sequence Diagram（时序图）`
   - 说明专家协同执行时，任务如何在主理人、流程模板、专家、技能之间流转
...
flowchart TB
    ...
    subgraph Execute["执行与协同层"]
        Orchestrator["task-orchestrator（任务主理人）"]
...
- `ai-spec sync（同步）`
  - 负责安装与求解
- `ai-spec run（运行）`
  - 负责编排与执行，当前默认由 `task-orchestrator（任务主理人）` 在 `IDE（开发工具） AI（智能体）` 环境中触发

也就是说：

> 先通过 `sync（同步）` 把能力装进目标项目，再通过 `task-orchestrator（任务主理人）` 触发 `run（运行编排）` 启动一次真实任务。
...
flowchart LR
    ...
    Run["run（运行编排）<br/>当前默认由 task-orchestrator（任务主理人）触发"]
...
| `sync（同步）` | 根据 `manifest（安装清单）` 做增量求解、安装和状态落盘 | `manifest（安装清单）`、覆盖参数 | `.ai-spec/manifest.json / lock.json / sources.json` | 契约已定义，命令待实现 |
| `run（运行编排）` | 读取任务输入，选择 `flow（流程模板）`，产出 `run-plan（运行计划）` 并逐步驱动专家协同 | 自然语言需求、`change（变更 ID）`、设计说明、可选 `flow（流程模板）` | `flow-descriptor（流程模板描述）`、`run-plan（运行计划）`、首轮桥接载荷、交接更新后的 `run-state（运行状态）`、后续交付产物 | 契约已定义，当前默认由 `task-orchestrator（任务主理人）` 触发；若上游为自然语言/Markdown（标记文本） 回复，则优先经 `task-orchestrator-extractor（输出抽取器）` 再进入 `task-orchestrator-adapter（自动执行适配层）` 落盘 |
...
flowchart LR
    ...
    Sync["sync（同步）<br/>按 manifest（安装清单）增量求解与同步"]
    Run["run（运行编排）<br/>由 task-orchestrator（任务主理人）读取任务输入并生成 run-plan（运行计划）"]
...
sequenceDiagram
    ...
    participant Orchestrator as task-orchestrator（任务主理人）
    ...
    User->>Runtime: 提交任务输入或显式触发 task-orchestrator（任务主理人）
...
- 当前只是“契约已定义”
- 当前默认运行入口应理解为 `IDE（开发工具） AI（智能体）` 或 OpenClaw（远程入口）中的 `task-orchestrator（任务主理人）`
...
```text
OpenClaw（远程入口） -> 触发 task-orchestrator（任务主理人） -> 任务主理人编排专家
```
...
- `task-orchestrator（任务主理人）` 与 `flow（流程模板）` 基础骨架
- `task-orchestrator-extractor（主理人输出抽取器）` 与 `task-orchestrator-adapter（主理人自动执行适配层）`
...
> 当前项目不是一个单独的 `skill（技能）` 仓库，也不是单纯的安装脚本，而是一个围绕 `规则（rule）`、`上下文（context）`、`流程模板（flow）`、`专家角色（role）` 和 `技能（skill）` 组织起来的 AI 规范驱动开发平台。Hub（平台）负责选择与分发，CLI（命令行工具）负责解析与安装，目标项目负责承接与消费，`task-orchestrator（任务主理人）` 负责在 `IDE（开发工具） AI（智能体）` 或远程入口中把这些能力编排成可执行、可验证、可扩展的协作链路。

## 13. 相关文档

- [AI规范驱动开发平台-综合蓝图.md](/Users/lizhenwei/workspace/vueworkspace/bairong/br-ai-spec/docs/paser_two/AI规范驱动开发平台-综合蓝图.md)
- [Hub平台融合与OpenClaw对接方案.md](/Users/lizhenwei/workspace/vueworkspace/bairong/br-ai-spec/docs/paser_two/Hub平台融合与OpenClaw对接方案.md)
