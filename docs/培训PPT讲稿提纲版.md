...
### 第 6 页 | br-ai-spec 是什么

**标题**：br-ai-spec — AI Coding 团队规范库
...
- 自动化层（configs/）：ESLint + Prettier + husky — 提交前检查
...
- Stylelint：样式规范检查
- commitlint：提交信息格式校验
- husky + lint-staged：只检查暂存文件，pre-commit 自动触发
...
- **13-格式化**：Prettier（单引号、2 空格、100 字符）+ ESLint + Stylelint + husky
...
**讲师备注**：
这是 br-ai-spec 最独特的规范。它确保 AI 不是"一股脑输出代码"，而是像有经验的开发者一样"先想清楚再动手"。
...
| 类别 | 技能 | 数量 |
|------|------|------|
| 流程类 | using-superpowers、create-proposal、execute-task | 3 |
...
**讲师备注**：
create-proposal 是"需求入口"，它会根据条件自动决定调用哪些子技能。
...
**讲师备注**：
这些问题不需要当场全部回答，但接入前必须对齐。br-ai-spec 默认提供了一套答案（Vue: views + Element Plus + SCSS Modules + api/ + Pinia + 手写路由），团队可以根据实际调整。
...
**要点**：
- 规范库：`git clone http://git.100credit.cn/zhenwei.li/br-ai-spec.git`
