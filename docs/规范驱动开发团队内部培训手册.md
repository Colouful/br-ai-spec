...
> ex-ai-spec v2.0 | AI Coding 团队规范驱动开发 CLI（**前端**：Vue 3 / React Profile）
...
```bash
npx @ex/ai-spec init
npx @ex/ai-spec init --profile vue              # 默认 L3（含 OpenSpec）
npx @ex/ai-spec init --profile vue --level L2   # 不要 OpenSpec 时
npx @ex/ai-spec init --profile react            # 默认 L3
```
