...
> br-ai-spec v2.0 | AI Coding 团队规范库
...
- 使用 AI IDE（Cursor / Claude Code / OpenCode / Trae）进行日常开发的前端团队
- Vue 3 或 React 技术栈
...
graph TB
    ...
    subgraph automationLayer ["自动化层 configs/"]
        Configs["ESLint + Prettier + Stylelint<br/>+ husky + commitlint"]
    end
...
| **流程层** | `openspec/` | 管理需求到归档的完整流程 | 项目管理 |
...
提交前自动执行的代码检查，确保基础风格一致：

| 工具 | 职责 | 触发时机 |
|------|------|----------|
| **Prettier** | 格式化（缩进、引号、分号） | pre-commit |
| **ESLint** | 代码质量与风格检查 | pre-commit |
| **Stylelint** | CSS/SCSS 样式检查 | pre-commit |
| **commitlint** | Git 提交信息格式校验 | commit-msg |
| **husky** | Git hooks 管理器 | 自动 |
...
| **L2** | `.agents` + IDE 适配 + MCP 模板 | 团队标准接入（推荐） |
| **L3** | L2 + OpenSpec 流程 | 需要需求治理与归档 |
...
| **create-proposal** | "创建提案"、"新增需求" | 明确类型 → 设计分析 → 组件规划 → 接口对接 → 输出 proposal/tasks/spec |
...
```
1. 流程类技能优先：create-proposal → execute-task
...
AI 会触发 `create-proposal` 技能，按以下步骤执行：
...
| 创建提案 | "帮我创建一个变更提案" | create-proposal |
...
**Q: OpenSpec 是必须的吗？**
A: 不是。L1/L2 级别不包含 OpenSpec。仅在需要需求治理时使用 L3 级别。
