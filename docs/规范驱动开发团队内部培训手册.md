...
> ex-ai-spec v2.0 | AI Coding 团队规范库（**前端**：Vue 3 / React Profile）
...
ex-ai-spec 通过约束、操作、自动化与流程四层协同工作；其中 **流程层 `openspec/` 仅在 L3 安装后存在**，并与 `.agents` 通过 **`openspec/config.yaml`** 桥接。
...
| **流程层** | `openspec/`（L3） | 管理需求到归档；`config.yaml` 注入 ex-ai-spec 上下文 | 项目管理 |
...
**与 ex-ai-spec 的衔接**：安装 **L3** 时生成的 **`openspec/config.yaml`** 会配置 `context` / `rules`，使 **`/opsx:*` 流程**自动读取 **`.agents`** 中的规范与技能；提案产物位于 **`openspec/changes/`**（及 `specs/` 等），与仓库内其他目录共同构成一体能力。
...
| **L2** | `.agents` + IDE 适配 + MCP 模板 | 团队编码规范 + 外部上下文（默认安装层级） |
| **L3** | L2 + OpenSpec（`openspec/`、OPSX） | **团队完整方案**：需求治理与归档，与 `.agents` 经 `config.yaml` 一体联动 |
...
npx @ex/ai-spec init --profile react --level L3   # 完整方案：含 OpenSpec
```
...
等价命令：在项目根目录执行 `npx @ex/ai-spec init`、`npx @ex/ai-spec update`、`npx @ex/ai-spec check`、`npx @ex/ai-spec uninstall`。
...
若使用 **`--level L3`**，确认已生成 **`openspec/`**（含 `config.yaml`），并阅读仓库 [docs/openspec-guide.md](openspec-guide.md)。**`create-proposal` 委托的 `/opsx:propose`** 及 **`openspec/changes/`** 下提案产物 **仅在此层级可用**；L1/L2 下可日常开发，但不具备完整 OPSX 流程目录。
...
| **project-init** | "初始化项目规范" | 读取项目信息 → 生成 01-项目概述 + 03-项目结构 |
...
**Q: 安装后 AI 没有遵循规范？**
A: 运行 `npx @ex/ai-spec check` 或 `install.sh check` 确认链接有效。部分 IDE 需要重启才能识别新的规则文件。
...
**Q: OpenSpec 是必须的吗？**
A: **安装层级上可选**：L1/L2 不包含 `openspec/` 与 OPSX 工作流。若团队要做 **规范驱动的前端交付闭环**（提案、拆分、实施、归档），建议采用 **L3**，与 ex-ai-spec **同一安装流程**落地，经 `config.yaml` 与 `.agents` 联动；日常小改仍可直接对话开发。
