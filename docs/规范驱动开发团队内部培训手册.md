...
| **create-proposal** | "创建提案"、"新增需求" | 需求前置分析 → 设计分析（可选）→ 委托 `/opsx:propose` 生成提案 → 后置检查 |
...
AI 会触发 `create-proposal` 技能进行前置分析，然后委托 `/opsx:propose` 生成提案：
...
| 创建提案 | "帮我创建一个变更提案" | create-proposal（→ /opsx:propose） |
