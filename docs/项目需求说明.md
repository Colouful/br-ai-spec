...
ex-ai-spec 是一套 **AI Coding 团队规范库（当前版本面向前端）**，内置 **Vue 3** 与 **React** 两套 Profile，旨在让 Cursor、Claude Code、OpenCode、Trae 等 AI 编码助手在 **前端工程** 中自动遵循统一的开发规范、工作流程和最佳实践。
...
| L3 | L2 + OpenSpec（`openspec/`、OPSX） | **团队完整方案**：需求治理与归档，与 `.agents` 一体联动 |
...
- OpenSpec（**L3**）：管理需求 propose → apply → archive 全流程；**`openspec/config.yaml`** 注入 ex-ai-spec 的 `context` / `rules`，与 **`.agents`** 协同
...
```mermaid
graph TB
    subgraph specRepo ["规范源仓库 ex-ai-spec "]
