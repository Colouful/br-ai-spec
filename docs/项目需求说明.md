...
br-ai-spec 是一套 **AI Coding 团队规范库**，旨在让 Cursor、Claude Code、OpenCode、Trae 等 AI 编码助手自动遵循统一的开发规范、工作流程和最佳实践。
...
**解决方式**：
- `create-proposal` 技能：前置分析后委托 `/opsx:propose` 在 `openspec/changes/` 下生成提案
- `execute-task` 技能：按 tasks.md 逐条执行
- `ui-verification` 技能：实现后 UI 验收
- OpenSpec（L3）：管理需求 propose → apply → archive 全流程
...
```mermaid
graph TB
    subgraph specRepo ["规范源仓库 br-ai-spec"]
        ...
        Configs["configs/<br/>ESLint/Prettier/Stylelint/husky"]
...
│           └── vue/                  # 6 个 Vue 技能
├── configs/
│   ├── common/                       # 共享配置（prettier/stylelint/husky/commitlint）
│   └── profiles/
